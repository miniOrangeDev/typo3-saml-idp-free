...
use Miniorange\Idp\Helper\Lib\XMLSecLibs\XMLSecEnc;
use Miniorange\Idp\Helper\Lib\XMLSecLibs\XMLSecurityDSig;
...
class SAMLUtilities extends Utilities
{
     ...
     * @param $sloUrl
     */
    public static function sendHTTPPostRequest($samlRequest, $sendRelayState, $sloUrl)
    {
        $privateKeyPath = file_get_contents(__DIR__ . '/resources/sp-cert.key');
        $publicCertPath = file_get_contents(__DIR__ . '/resources/sp-cert.crt');
    ...
    public static function mo_saml_miniorange_generate_metadata($sp_name, $download = false)
    {
        $saml_login_url = self::fetchFromTable('saml_login_url', Constants::TABLE_SAML, $sp_name);
        $entity_id = self::fetchFromTable('idp_entity_id', Constants::TABLE_SAML, $sp_name);
        if (ob_get_contents())
            ob_clean();
        ...
        echo '<?xml version="1.0"?>
                <md:EntityDescriptor xmlns:md="urn:oasis:names:tc:SAML:2.0:metadata" entityID="' . $entity_id . '" ID="_58eb6efc-1f19-431b-8146-3fef71f908d0">
                <md:IDPSSODescriptor ID="_04494e78-cc4f-404b-9fa8-16373b073228" protocolSupportEnumeration="urn:oasis:names:tc:SAML:2.0:protocol" WantAuthnRequestsSigned="true">
                <md:KeyDescriptor use="signing">
                <KeyInfo xmlns="http://www.w3.org/2000/09/xmldsig#">
                <X509Data>
                <X509Certificate> MIID/zCCAuegAwIBAgIJAMePaIrAItqFMA0GCSqGSIb3DQEBCwUAMIGVMQswCQYD
                ...
                <md:SingleSignOnService Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect" Location="' . $saml_login_url . '"/>
                <md:SingleSignOnService Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST" Location="' . $saml_login_url . '"/>
                </md:IDPSSODescriptor>
                </md:EntityDescriptor>';
        exit;
